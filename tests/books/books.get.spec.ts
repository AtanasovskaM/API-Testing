...
import { alphanumBookId, bookId, invalidId, negativeBookId } from '../../helpers/fixtures/books';
...
test('Get book by id with alphanumeric format', async ({ request }) => {
  ...
  expect(responseBody.errors.id[0]).toBe(`The value '${alphanumBookId}' is not valid.`);
});
