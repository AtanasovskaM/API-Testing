...
test.describe("PUT operation - books", () => {
  ...
  test("Update existing book with valid data", async ({ request }) => {
    ...
    expect(responseBody).toHaveProperty("title", updateBook.title);
    expect(responseBody).toHaveProperty("description", updateBook.description);
    expect(responseBody).toHaveProperty("excerpt", updateBook.excerpt);
  });
